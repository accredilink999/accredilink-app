...
export default function ForumHeader({ profile, token, onLogout }) {
  ...
  return (
    <header className="bg-slate-900/85 backdrop-blur-xl border-b border-slate-700/60 sticky top-0 z-40 shadow-lg">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <nav className="hidden md:flex items-center gap-1 ml-1">
              <Link href="/forum" className={`px-3 py-1.5 text-sm rounded-lg transition-colors font-semibold flex items-center gap-1.5 ${isForumHome ? 'text-teal-300 bg-teal-500/15' : 'text-slate-200 hover:text-teal-300 hover:bg-white/10'}`}>
                <Home className="w-3.5 h-3.5 text-teal-400" /> Home
              </Link>
              {profile && canModerate(profile.forum_role) ? (
                <Link href="/forum/admin" className={`px-3 py-1.5 text-sm rounded-lg transition-colors font-semibold flex items-center gap-1.5 ${pathname.startsWith('/forum/admin') ? 'text-amber-300 bg-amber-500/15' : 'text-amber-300 hover:text-amber-200 hover:bg-amber-500/10'}`}>
                  <Shield className="w-3.5 h-3.5 text-amber-400" /> Site Admin
                </Link>
              ) : (
                <Link href="/forum/members" className="px-3 py-1.5 text-sm text-slate-200 hover:text-blue-300 hover:bg-white/10 rounded-lg transition-colors font-medium flex items-center gap-1.5">
                  <Users className="w-3.5 h-3.5 text-blue-400" /> Members
                </Link>
              )}
              <Link href="/" className="px-3 py-1.5 text-sm text-slate-300 hover:text-purple-300 hover:bg-white/10 rounded-lg transition-colors font-medium flex items-center gap-1.5">
                <ArrowLeft className="w-3.5 h-3.5 text-purple-400" /> Main Site
              </Link>
            </nav>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            {profile ? (
              <>
                <Link href="/forum/messages" className="p-2 text-blue-400 hover:text-blue-300 hover:bg-blue-500/10 rounded-lg transition-colors hidden sm:flex" title="Messages">
                  <Mail className="w-5 h-5" />
                </Link>
                <NotificationBell token={token} />
                <PushSubscription token={token} />
                <Link href="/forum/settings" className="p-2 text-purple-400 hover:text-purple-300 hover:bg-purple-500/10 rounded-lg transition-colors hidden sm:flex" title="Settings">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden mt-3 pt-3 border-t border-white/10">
            ...
            <div className="flex flex-col gap-1 pb-2">
              <Link href="/forum" className="px-3 py-2 text-sm text-slate-200 hover:bg-white/10 rounded-lg font-semibold flex items-center gap-2"><Home className="w-4 h-4 text-teal-400" /> Home</Link>
              {profile && canModerate(profile.forum_role) ? (
                <Link href="/forum/admin" className="px-3 py-2 text-sm text-amber-300 hover:bg-amber-500/10 rounded-lg font-semibold flex items-center gap-2"><Shield className="w-4 h-4 text-amber-400" /> Site Admin</Link>
              ) : (
                <Link href="/forum/members" className="px-3 py-2 text-sm text-slate-200 hover:bg-white/10 rounded-lg font-medium flex items-center gap-2"><Users className="w-4 h-4 text-blue-400" /> Members</Link>
              )}
              <Link href="/forum/messages" className="px-3 py-2 text-sm text-slate-200 hover:bg-white/10 rounded-lg font-medium flex items-center gap-2"><Mail className="w-4 h-4 text-blue-400" /> Messages</Link>
              <Link href="/forum/settings" className="px-3 py-2 text-sm text-slate-200 hover:bg-white/10 rounded-lg font-medium flex items-center gap-2"><Settings className="w-4 h-4 text-purple-400" /> Settings</Link>
              <Link href="/" className="px-3 py-2 text-sm text-slate-300 hover:bg-white/10 rounded-lg font-medium flex items-center gap-2"><ArrowLeft className="w-4 h-4 text-purple-400" /> Main Site</Link>
            </div>
          </div>
        )}
      </div>
