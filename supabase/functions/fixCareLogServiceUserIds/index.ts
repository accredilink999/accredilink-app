...
Deno.serve(async (req) => {
  ...
  if (authError || !currentUser) return Response.json({ error: 'Unauthorized' }, { status: 401 });
    ...
    // Get all care logs and service users
    ...
    // Extract unique old IDs with their names from care logs
    const oldIdToNewId = {};
    const updates = [];
    ...
    // Update all care logs with correct service user IDs
    for (const log of careLogs) {
      ...
      if (newId && newId !== oldId) {
        ...
        // Update the care log
        await (async () => { const { data, error } = await supabaseAdmin.from('care_logs').update({
