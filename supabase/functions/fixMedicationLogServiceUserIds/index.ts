...
Deno.serve(async (req) => {
  ...
  if (authError || !currentUser) return Response.json({ error: 'Unauthorized' }, { status: 401 });
    ...
    // Get all medication administration logs and service users
    const medLogs = await (async () => { const { data, error } = await supabaseAdmin.from('medication_administrations').select('*').order('created_at', { ascending: false }).limit(1000); if (error) throw error; return data || [] })();
    ...
    medLogs.forEach(log => {
      const oldId = log.service_user_id;
      const oldName = log.service_user_name?.trim() || '';
    ...
    // Update all medication logs with correct service user IDs
    for (const log of medLogs) {
      ...
      if (newId && newId !== oldId) {
        updates.push({
          ...
          oldId,
          newId,
          name: log.service_user_name
        });

        await (async () => { const { data, error } = await supabaseAdmin.from('medication_administrations').update({
