...
// Call-end tone — same Bop Beep as PTT release
function playCallEndTone() {
  if (localStorage.getItem('radio_silent_mode') === 'true') return;
  try {
    const stored = localStorage.getItem('radio_custom_sounds');
    if (stored) {
      const snd = JSON.parse(stored)['call_end'];
...
export default function TwoWayRadio() {
  ...
  // Mic pre-warm for PWA mode (non-radio) — on first screen touch, create the track
  // so it exists before any PTT press. Radio mode handles this via the activation splash.
  useEffect(() => {
    if (isRadioMode) return; // splash handles it for radio mode
    const prewarm = async () => {
