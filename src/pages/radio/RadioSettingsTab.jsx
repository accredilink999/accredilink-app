...
const ALERTER_VOLUMES = [
  { id: 'high',   label: 'High',   value: 1.0 },
  { id: 'medium', label: 'Medium', value: 0.5 },
  { id: 'low',    label: 'Low',    value: 0.25 },
];

function AlerterVolumePicker() {
  const [selected, setSelected] = useState(() => localStorage.getItem('alerter_volume') || 'high');

  const pick = (id) => {
    setSelected(id);
    localStorage.setItem('alerter_volume', id);
  };

  return (
    <div className="space-y-1 pt-1">
      <p className="text-slate-400 text-xs font-semibold">Alert Volume</p>
      <div className="flex gap-2">
        {ALERTER_VOLUMES.map(v => (
          <button
            key={v.id}
            onClick={() => pick(v.id)}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors ${selected === v.id ? 'bg-teal-600 border-teal-500 text-white' : 'bg-slate-800 border-slate-700 text-slate-400'}`}
          >
            {selected === v.id && <Check className="w-3 h-3" />}{v.label}
          </button>
        ))}
      </div>
    </div>
