...
// Realistic medical body outline SVG paths (from react-native-body-highlighter, MIT license)
...
function BodySvg({ side, markers, selectedIndex, onPointerDown, onMarkerClick, readOnly, containerRef }) {
  ...
  const handleSvgPointerDown = useCallback((e) => {
    if (readOnly || !onPointerDown) return;
    ...
    // Normalize to 0-1
    const xOffset = side === 'front' ? 0 : 724;
    const nx = (svgPt.x - xOffset) / 724;
    const ny = svgPt.y / 1448;
    const cx = Math.max(0, Math.min(1, nx));
    const cy = Math.max(0, Math.min(1, ny));
    onPointerDown(cx, cy, side);
  }, [readOnly, onPointerDown, side]);
  ...
  return (
    <div className="flex-1 flex flex-col items-center">
      ...
      <svg
        ref={side === 'front' ? containerRef : undefined}
        viewBox={viewBox}
        className="w-full"
        style={{ maxHeight: 400, touchAction: 'none' }}
        preserveAspectRatio="xMidYMid meet"
        onPointerDown={handleSvgPointerDown}
      >
        {/* Body fill - light skin tone area */}
        <path
          d={outline}
          fill="#fce4c8"
          stroke="none"
        ...
        {/* Marker overlays rendered in SVG space */}
        {sideMarkers.map((marker) => {
          const xOffset = side === 'front' ? 0 : 724;
          const svgX = xOffset + marker.x * 724;
          const svgY = marker.y * 1448;
          const isSelected = selectedIndex === marker._globalIndex;
          return (
            <g
              key={marker._globalIndex}
              onPointerDown={(e) => { e.stopPropagation(); onMarkerClick(e, marker._globalIndex); }}
            ...
            >
              ...
              {/* Red cross mark */}
              <line
                x1={svgX - (isSelected ? 14 : 10)}
                y1={svgY - (isSelected ? 14 : 10)}
                x2={svgX + (isSelected ? 14 : 10)}
                y2={svgY + (isSelected ? 14 : 10)}
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <span className="text-xs text-slate-500">
          ...
          {!readOnly && ' — tap body to mark area of concern'}
        </span>
      </div>
      ...
      {markers.length > 0 && (
        <div className="space-y-1">
          {markers.map((m, i) => (
            <div
              key={i}
              className={`text-xs px-3 py-1.5 rounded cursor-pointer transition-colors flex items-center gap-2
                ${selectedIndex === i ? 'bg-red-50 text-red-800 border border-red-200' : 'text-slate-600 hover:bg-slate-50'}`}
